...
public class AliasTableInfoCommand : AliasServerAdminCommandBase
{
	...
	public override async Task Callback(SocketSlashCommand arg, UserData data, DataBaseService.DbDataRequester requester, object executer)
	{
		...
		await RouteSubCommand(arg.Data,
			new(nameof(Operation.Get).ToLower(), HandleGet),
			new(nameof(Operation.Set).ToLower(), HandleSet));

		async Task HandleGet(SocketSlashCommandDataOption option)
		{
			string returnString = $"""
		...
		async Task HandleSet(SocketSlashCommandDataOption option)
		{
			Field field = option.GetOption<Field>("field");
			string valueString = option.GetOption<string>("value");
			switch (field)
			{
